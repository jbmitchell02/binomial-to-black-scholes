...
# Functions for calculating the price of a European call option
...
# sigma: Standard deviation of stock returns
# r_f: Risk-free rate
def black_scholes_price(S, K, T, sigma, r_f):
    d1 = math.log(S / K) + T * (r_f + (pow(sigma, 2) / 2))
    d1 /= (sigma * math.sqrt(T))
    d2 = d1 - (sigma * math.sqrt(T))
    N1 = norm.cdf(d1)
    N2 = norm.cdf(d2)
    return (S * N1 - K * pow(math.e, -r_f * T) * N2)
...
if __name__ == '__main__':
    ...
    sigma = 0.1
    r_f = 0.01
    bin_price = binomial_price(S, K, n, T, sigma, r_f)
    bs_price = black_scholes_price(S, K, T, sigma, r_f)
    print('The', n, 'period binomial model price is $', bin_price)
